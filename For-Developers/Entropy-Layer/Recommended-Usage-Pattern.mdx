...
The Entropy Layer SDK separates data fetching from transaction execution. A typical user journey for a Liquidity Provider involves checking the current pool state, verifying their token permissions, building the deposit calldata, and executing the transaction via a smart account.
